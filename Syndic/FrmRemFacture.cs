...
namespace Syndic
{
    public partial class FrmRemFacture : Form
    {
        ...
        private void remplirLst()
        {
            ...
            string sql = "select * from remarque_facture where id_facture = " + pos;
        ...
        {
            bsFac = Fonctions.remplirList(cb_facture, "facture", "designation", "id_facture");
            ...
            switch (btn.Text)
            {
                ...
                case "Supprimer":
                    if (lst_Remaques.Items.Count > 0)
                    {
                        ...
                        {
                            cmd = new SqlCommand("delete from remarque_facture where id_remaque = " + lst_Remaques.SelectedValue + "", Fonctions.CnConnection());
                            cmd.ExecuteNonQuery();
                ...
                case "Valider":
                    if (ajt)
                    {
                        cmd = new SqlCommand("insert into remarque_facture values ('" + txt_nomremarque.Text + "','" + txt_remarque.Text + "'," + cb_facture.SelectedValue + ")", Fonctions.CnConnection());
                        cmd.ExecuteNonQuery();
